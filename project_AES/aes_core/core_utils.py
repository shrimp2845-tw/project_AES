...
def shiftrow(data: list[int]) -> list[int]:
    ...
    t = [0, 5, 10, 15,
          ...
          12, 1, 6, 11]
    if len(data) != 16:
        raise ValueError('shiftrow: invalid data size')
...
def mixcolumns(data: list[int]) -> list[int]:
    ...
    def mc(column: list[int]) -> list[int]:
        ...
        s0, s1, s2, s3 = column
        r0 = gfm(0x02, s0) ^ gfm(0x03, s1) ^ s2 ^ s3
        r1 = s0 ^ gfm(0x02, s1) ^ gfm(0x03, s2) ^ s3
        r2 = s0 ^ s1 ^ gfm(0x02, s2) ^ gfm(0x03, s3)
        r3 = gfm(0x03, s0) ^ s1 ^ s2 ^ gfm(0x02, s3)
        return [r0, r1, r2, r3]
    if len(data) != 16:
        raise ValueError('mixcolumns: invalid data size')
    cl = [data[i: i+4] for i in range(0, 16, 4)]
    result = [i for j in cl for i in mc(j)]
    return result
...
def inv_mixcolumns(data: list[int]) -> list[int]:
    ...
    def inv_mc(column: list[int]) -> list[int]:
        ...
        r3 = gfm(0x0B, s0) ^ gfm(0x0D, s1) ^ gfm(0x09, s2) ^ gfm(0x0E, s3)
        return [r0, r1, r2, r3]
    if len(data) != 16:
        raise ValueError('inv_mixcolumns: invalid data size')
    cl = [data[i: i+4] for i in range(0, 16, 4)]
    result = [i for j in cl for i in inv_mc(j)]
    return result
...
def sbox(data: int, table: list[int] = None) -> int:
    ...
    if table and len(table) != 256:
        raise ValueError('sbox: invalid table size')
    if not table:
        table = SBOX
    result = table[data]
    return result
...
def inv_sbox(data: int, table: list[int] = None) -> int:
    ...
    if table and len(table) != 256:
        raise ValueError('inv_sbox: invalid table size')
    if not table:
        table = INV_SBOX
    result = table[data]
    return result
...
def subbytes(data: list[int], tb: list[int] = None) -> list[int]:
    ...
    if tb and len(tb) != 256:
        raise ValueError('subbytes: invalid table size')
    result = [sbox(i, table = tb) for i in data]
    return result
